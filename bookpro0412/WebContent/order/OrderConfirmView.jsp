...
<table border=1 width=1000 align=center>
		...
		<%-- <c:if test="${b.order_status=='배송대기'}"> --%>
			...
			<tr>
			 ...
			 onclick="location.href='./OrderUpdate.odo?order_num=${b.order_num}'"></c:if>
			 </th>		
...
<!-- 1페이지로 이동 -->
<a href="./OrderConfirm.odo?page=1" style="text-decoration:none"> << </a>

<!-- 이전 블럭으로 이동 -->
<c:if test="${startPage > 10}">
	<a href="./OrderConfirm.odo?page=${startPage-10}">[이전]</a>
</c:if>
...
<c:forEach var="i" begin="${startPage}" end="${endPage}">
	...
	<c:if test="${i != page}">	<!-- 현재 페이지가 아닌 경우 -->
		<a href="./OrderConfirm.odo?page=${i}">[${i}]</a>
	</c:if>
</c:forEach>

<!-- 다음 블럭으로 이동 -->
<c:if test="${endPage < pageCount}">
	<a href="./OrderConfirm.odo?page=${startPage+10}">[다음]</a>
</c:if>

<!-- 마지막 페이지로 이동 -->
<a href="./OrderConfirm.odo?page=${pageCount}" 
...
<%-- <br><br>
<center>
<c:forEach var="b1" items="${orderlist}">
<c:if test="${b1.order_status=='배송대기'}">	<!-- 관리자 아이디 : admin으로 로그인 할시 출력 -->
<div class="header">
      
      <ul id="Login-status">
       
       <li><input type=button value="test"
			 onclick="location.href='./OrderUpdate.odo?order_num=${b.order_num}'"><li>
