...
$(document).ready(function(){
	...
	<c:forEach var="s" items="${h}"> //request.setAttribute("h", h); 에 들어가는 앞의 h값이 들어가는 것임 바로 배열이 들어가는것이 아님
...
<table border=1 width=500 align=center>
	...
	<tr><td>ID</td>
		...
		%>
		<%=mem.getId() %><br>
<!-- 여기서 member는 UpdateMember.java에서 request객체에서 세션 공유된 값이다, el의 편의성 확인-->		
			${member.id } 
		</td>
	</tr>
	<tr><td>비밀번호</td>
	...
	<tr><td>핸드폰</td>
		<td><select id="phone1" name="phone1">
				...
				<option value="010"  
				<c:if test="${member.phone1 =='010'}">${'selected'}</c:if>				
				>010</option>
				
				<option value="011"
				<c:if test="${member.phone1 =='011'}">${'selected'}</c:if>
				>011</option>
				
				<option value="016"
				<c:if test="${member.phone1 =='016'}">${'selected'}</c:if>
				>016</option>
				
				<option value="018"
				<c:if test="${member.phone1 =='018'}">${'selected'}</c:if>
				>018</option>
				
				<option value="019"
				<c:if test="${member.phone1 =='019'}">${'selected'}</c:if>
				>019</option>
				
			</select>-
			<input type=text size=4 id="phone2" name="phone2" 
					maxlength=4 value="${member.phone1}">-
